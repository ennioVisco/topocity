...
sGeometry i = AbstractGeometry (GML (Just i) [] Nothing) Nothing

l0f = Just (FootPrint fp)
l0r = Just (RoofEdge  re)
l1s = Just (BldgLod1Solid (Solid (sGeometry "RandomSolid") cs []))

fp = MultiSurface (AbstractGeometry (GML Nothing [] Nothing) Nothing)
    [
        Polygon (AbstractGeometry (GML Nothing [] Nothing) Nothing)
            (LinearRing (sGeometry "RandomID")
...
re = MultiSurface (AbstractGeometry (GML Nothing [] Nothing) Nothing)
    [
        Polygon (AbstractGeometry (GML Nothing [] Nothing) Nothing)
            (LinearRing (sGeometry "RandomID")
...
cs = CompositeSurface (sGeometry "randomSurf")
    [
        Polygon (AbstractGeometry (GML Nothing [] Nothing) Nothing)
            (LinearRing (sGeometry "RandomID")
              [  Coord "85017.737 447228.101 0.000"  Nothing
