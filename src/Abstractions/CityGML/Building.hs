...
instance Abstractable WallSurface where
    absObj w@(WallSurface f _ _ os) = NTree (uid f, ("WallSurface", pack $ show w))
                                            (map absObj os)

    reiObj (NTree (_, ("WallSurface", d)) os)
        = reshape' (read $ unpack d) os
            where
            reshape' (WallSurface f l2 l3 _) os =
                    WallSurface f l2 l3 (map reiObj os)

instance Abstractable RoofSurface where
    absObj w@(RoofSurface f _ _ os) = NTree (uid f, ("RoofSurface", pack $ show w))
                                            (map absObj os)
...
instance Abstractable BldgBoundary where
    ...
    absObj n        = NTree (uid n, (show $ toConstr n, pack $ show n)) []
