...
module HXTExtras (xpAnyElem, xpAnyAttr) where

import           Control.Monad.State              (gets, modify)
import           Text.XML.HXT.Arrow.Pickle        (PU (PU, appPickle, appUnPickle, theSchema),
                                                   xpAttr, xpCheckEmpty,
                                                   xpCheckEmptyContents, xpElem)
import           Text.XML.HXT.Arrow.Pickle.Schema (Schema (Any))
import           Text.XML.HXT.Arrow.Pickle.Xml    (Unpickler, attributes,
                                                   getCont, liftMaybe,
                                                   liftUnpickleVal,
                                                   liftUnpickleVal, nesting,
                                                   throwMsg, unpickleElem')
